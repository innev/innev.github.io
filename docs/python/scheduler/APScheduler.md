...
BlockingScheduler 是 APScheduler 中的调度器，APScheduler 中有两种常用的调度器，BlockingScheduler 和 BackgroundScheduler，当调度器是应用中唯一要运行的任务时，使用 BlockingSchedule，如果希望调度器在后台执行，使用 BackgroundScheduler。
    ...
    2. BackgroundScheduler: use when you’re not using any of the frameworks below, and want the scheduler to run 3. in the background inside your application
    4. AsyncIOScheduler: use if your application uses the asyncio module
    6. GeventScheduler: use if your application uses gevent
    7. TornadoScheduler: use if you’re building a Tornado application
    8. TwistedScheduler: use if you’re building a Twisted application
    9. QtScheduler: use if you’re building a Qt application
...
![python 定时任务](python_timer_expression.png)
